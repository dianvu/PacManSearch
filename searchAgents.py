...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        return self.startingPosition, () # visited_corners initialized as empty tuple - as not visited any node
    ...
    def isGoalState(self, state):
        ...
        if len(self.corners) != len(state[1]):
            return False # not all corners visited
        return True      # all corners visited
        util.raiseNotDefined()
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                # mark as nextState
                nextState = (nextx, nexty)
                # check if nextState is a corner and not visited
                if nextState in self.corners and nextState not in state[1]:
                    # mark nextState as visited
                    visited += (nextState,)
                # add nextState to successors with cost of 1
...
def cornersHeuristic(state, problem):
    ...
    # initialize result and position
    result = 0
    pos = state[0] # current position
    # get remaining corner in prolem.corners but not visited
    remaining_corners = [corner for corner in corners if corner not in state[1]]
    # loop until all corners are visited
    while len(remaining_corners) > 0:
        # distance from current position to all remaining corners which are not visited
        distance_array = [util.manhattanDistance(pos, corner) for corner in remaining_corners]
        # get the minimum distance and its index
        distance_min = min(distance_array)
        distance_index = distance_array.index(distance_min)
        # position is updated to the corner with minimum distance
        pos = remaining_corners[distance_index]
        # remove the corner from remaining_corners
        remaining_corners.pop(distance_index)
        # add the minimum distance to result
        result += distance_min
    return result
...
def foodHeuristic(state, problem):
    ...
    if len(myFood) == 0:
        return 0
    # if only one food left, return the distance to that food
    for x, y in myFood:
        pos = (x, y)
        distance.append(mazeDistance(position, pos, problem.startingGameState)) # mazeDistance return shortest path distance between position and food position with current state of game
    return max(distance) # return the maximum distance to food
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        return search.breadthFirstSearch(problem) # return the path to the closest dot
        util.raiseNotDefined()
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        # use Mahattan distance to find the tuple with smallest distance
